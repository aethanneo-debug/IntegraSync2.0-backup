...
const re = /<span className="font-semibold text-slate-500">Fund-Utilization Status:<\/span>\s*<span className="font-black text-slate-800">\{b\.budgetPercentageUsed\}% spent<\/span>\s*<\/div>\s*<div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">\s*<div\s*className=\{\`h-full rounded-full \$\{isOverspent \? "bg-rose-500" : b\.budgetPercentageUsed > 75 \? "bg-amber-500" : "bg-blue-600"\}\`\}\s*style=\{\{ width: \`\$\{Math\.min\(b\.budgetPercentageUsed, 100\)\}%\` \}\}\s*\/>\s*<\/div>\s*<\/div>\s*<\/div>/g;

content = content.replace(re, replacement);
fs.writeFileSync('src/components/FinanceView.tsx', content);
console.log("Budget Cap UI added to the second chart! (using regex)");
