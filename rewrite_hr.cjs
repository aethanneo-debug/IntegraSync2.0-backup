...
content = content.replace('const [bulkActionType, setBulkActionType] = useState<"verify" | "return" | null>(null);', 
...
content = content.replace('import { Check, Undo2, RefreshCcw } from "lucide-react";', 
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 space-y-3">
                  <div className="flex space-x-2">
                    <button 
                      ...
                      className={\`flex-1 py-2 text-xs font-bold rounded-lg border \${modalActionType === "return" ? "bg-amber-500 text-white border-amber-600" : "bg-white text-slate-600 border-slate-200 hover:bg-slate-50"}\`}
                    >
                      Reject / Return
                    </button>
                    <button 
                      onClick={() => setModalActionType("verify")} 
                      className={\`flex-1 py-2 text-xs font-bold rounded-lg border \${modalActionType === "verify" ? "bg-blue-600 text-white border-blue-700" : "bg-white text-slate-600 border-slate-200 hover:bg-slate-50"}\`}
                    >
                      Endorse to Chief
                    </button>
                  </div>
