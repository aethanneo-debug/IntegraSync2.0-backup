...
code = code.replace(
  ...
  'const [newBudgetCapVal, setNewBudgetCapVal] = useState("");\n  const [editingBudgetBreakdown, setEditingBudgetBreakdown] = useState({ ps: "", mooe: "", co: "" });'
);

// We need to replace the edit forms in BOTH places:
// Place 1:
...
// Since the block appears twice (or similar), we will replace both by replacing all occurrences
// Note: We need to use split/join if they match exactly.
