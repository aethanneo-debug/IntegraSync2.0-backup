...
const targetTop = `<div className="flex items-center justify-between">
...
content = content.replace(targetTop, `<h2 className="text-xs font-bold text-slate-700 uppercase font-mono tracking-wider">My Filed Requests Ledger</h2>`);

const targetRow = `{r.status}
                        </span>
                      </div>
                    </div>`;

const replacementRow = `{r.status}
                        </span>
                        {r.status === "Rejected" && (
                          <button
                            onClick={() => {
