...
const target = `                          <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                            <div 
...
const replacement = `                          <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                            <div 
                              className={\`h-full rounded-full \${isOverspent ? "bg-rose-500" : b.budgetPercentageUsed > 75 ? "bg-amber-500" : "bg-blue-600"}\`}
                              style={{ width: \`\${Math.min(b.budgetPercentageUsed, 100)}%\` }}
                            />
...
if (content.includes(target)) {
    ...
    console.log("Target not found in the second chart!");
}
